...
# 下拉式：贴屏幕顶部、留 24px 边距、高度 600，宽度保持窗口当前值
[pads.music]
app_id = '^mpv'
title = '^mpv —'
...
| `viewport` | 顶层 | 显示器分辨率（屏幕像素），driftwm 后端用于视野判断与全屏尺寸，默认 `[1920, 1080]` |
| `launch_wait_ms` | 顶层 | 自动启动后等待窗口出现的超时，默认 1500 |
...
| `width` / `height` | pad | 显示时的窗口尺寸（屏幕像素）；省略某一项则保持该方向的当前尺寸 |
...
- `width`/`height`/`margin` 都是**屏幕像素**，随当前缩放（zoom）自动换算成画布坐标；
...
- 几何配置在其他后端（foreign-toplevel）上会被忽略并给出提示。
...
- `0`：成功（包括"无窗口且未配置 launch"以外的一切正常路径）
- 非 `0`：出错（配置缺失/非法、合成器不支持协议、IPC 失败等），错误信息输出到 stderr
