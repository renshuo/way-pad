...
| 隐藏后屏幕表现 | 窗口原地透明，**看不见** | **原地留一个可见的占位窗口**（应用名/图标，点击或 Enter 重启）——并非从屏幕上消失 | 窗口移到画布远端藏匿点，看不见 |
| 应用进程 | 存活，秒级切换 | **被关闭**，显示/点击占位时经 .desktop 重启一个新进程 | 存活，秒级切换 |
| 应用状态 | 完整保留 | 取决于应用自身（zellij 会话 ✓、emacs ✗） | 完整保留 |
| 鼠标点击 | 透明窗口通常仍会拦截点击 | 占位窗口可正常交互 | 无此问题 |
